...
.DEFAULT_GOAL := help
.PHONY: help setup data bronze silver gold ml ai pipeline maintenance demo \
        rag report clean clean-data test lint
...
lint:  ## Verificacao rapida de sintaxe de todos os modulos
	$(PY) -m compileall -q src orchestration && echo "Sintaxe OK"
...
clean-data:  ## Apaga TODAS as camadas de dados (raw, bronze, silver, gold...)
	rm -rf data artifacts reports docs/generated
