...
def detect_anomalies(kpis: DataFrame, column: str = "tpv", threshold: float = 3.5) -> list[dict]:
    ...
    outliers = pdf[pdf["z_robusto"].abs() >= threshold].sort_values(
        "z_robusto", key=abs, ascending=False
    )
    return [
        {
...
def extract_credit_facts(portfolio: DataFrame) -> dict[str, Any]:
    ...
    by_product = (
        ...
        .agg(
            ...
            F.round(F.sum("npl_balance") / F.greatest(F.sum("ead"), F.lit(1.0)), 4).alias(
                "npl_ratio"
            ),
        )
        .orderBy(F.desc("ead"))
...
def extract_customer_facts(customers: DataFrame) -> dict[str, Any]:
    ...
    by_segment = (
        customers.groupBy("value_risk_segment")
        .agg(
            F.count("*").alias("clientes"),
            F.round(F.sum("tpv_90d"), 2).alias("tpv_90d"),
    ...
    by_rating = (
        customers.groupBy("rating").agg(F.count("*").alias("clientes")).orderBy("rating").collect()
    )
...
def extract_fraud_facts(signals: DataFrame, artifacts_dir: Path) -> dict[str, Any]:
    ...
    totals = (
        signals.agg(
            ...
            F.sum(
                F.when((F.col("fraud_score_rule") >= 45) & (F.col("is_fraud") == 1), 1).otherwise(0)
            ).alias("alertas_corretos"),
        )
        .collect()[0]
        .asDict()
    )
...
def extract_merchant_facts(merchants: DataFrame) -> dict[str, Any]:
    ...
    top = (
        merchants.groupBy("merchant_id", "merchant_name", "category")
        .agg(
            F.round(F.sum("tpv"), 2).alias("tpv"),
    ...
    risky = (
        ...
        .groupBy("merchant_id", "merchant_name", "category", "merchant_health")
        .agg(
            F.round(F.sum("tpv"), 2).alias("tpv"),
            F.round(F.avg("chargeback_rate"), 5).alias("taxa_chargeback"),
            F.round(F.avg("fraud_rate"), 5).alias("taxa_fraude"),
        )
        .orderBy(F.desc("taxa_chargeback"))
        .limit(5)
        .collect()
    )
    health = (
        merchants.groupBy("merchant_health")
        .agg(F.countDistinct("merchant_id").alias("lojistas"))
        .orderBy(F.desc("lojistas"))
        .collect()
    )
    return {
...
def collect_facts(spark: SparkSession) -> dict[str, Any]:
    ...
    facts = {
        ...
        "fraude": extract_fraud_facts(
            gold("transaction_fraud_signals"), cfg.layer_path("artifacts")
        ),
...
def render_markdown(facts: dict[str, Any]) -> str:
    ...
    add("# Relatorio analitico automatico - NeoPag")
    add("")
    add(
        f"*Gerado automaticamente pelo pipeline em {facts['gerado_em']} "
    ...
    add("")
    add(
        f"No periodo, a NeoPag processou **{_int(fin['transacoes'])}** transacoes, "
    ...
    add("")
    add(
        f"O take rate medio ficou em **{_pct(fin['take_rate'], 3)}** e a taxa de aprovacao "
        f"em **{_pct(fin['taxa_aprovacao'], 1)}**."
    )
    add("")
    if comp:
        add(
            f"Na comparacao de **{comp['mes_atual']}** contra **{comp['mes_anterior']}**: "
    ...
    for row in facts["financeiro"]["serie_mensal"]:
        add(
            f"| {row['year_month']} | {_money(row['tpv'])} | {_money(row['receita'])} | "
    ...
    if anomalies:
        ...
        for item in anomalies[:6]:
            add(
                f"- **{item['data']}**: `{item['metrica']}` = {_money(item['valor'])}, "
    ...
    add("")
    add(
        f"A carteira soma **{_money(cred['ead'])}** de exposicao (EAD) em "
    ...
    if facts["credito"]["piores_safras"]:
        ...
        for row in facts["credito"]["piores_safras"]:
            add(
                f"| {row['vintage']} | {row['product']} | {row['customer_risk_band']} | "
    ...
    add("")
    add(
        f"Sao **{_int(cust['clientes'])}** clientes ativos na base, com score medio de "
    ...
    for row in facts["clientes"]["por_segmento_de_valor"]:
        add(
            f"| {row['value_risk_segment']} | {row['clientes']} | {_money(row['tpv_90d'])} | "
            f"{row['score_medio']} |"
        )
    add("")
    ...
    add("")
    add(
        f"Foram identificadas **{_int(fraud['fraudes'])}** transacoes fraudulentas "
    ...
    if model:
        ...
        add(
            f"O modelo de ML (`{model.get('algoritmo')}`) atingiu **PR-AUC de "
    ...
    if facts["fraude"]["por_padrao"]:
        ...
        for row in facts["fraude"]["por_padrao"]:
            add(
                f"| {row['fraud_type'] or 'nao classificado'} | {row['ocorrencias']} | "
    ...
    if facts["lojistas"]["em_alerta"]:
        ...
        for row in facts["lojistas"]["em_alerta"]:
            add(
                f"| {row['merchant_name']} | {row['category']} | {row['merchant_health']} | "
    ...
    for row in facts["lojistas"]["maiores_por_tpv"]:
        add(
            f"- {row['merchant_name']} ({row['category']}): {_money(row['tpv'])} de TPV, "
    ...
    add("")
    add(
        "*Todos os numeros deste relatorio sao extraidos diretamente das tabelas Delta da "
...
def run() -> dict[str, Any]:
    ...
    with log_stage(log, "ai.insight_generator"):
        ...
        facts_path.write_text(
            json.dumps(facts, indent=2, ensure_ascii=False, default=str), encoding="utf-8"
        )
