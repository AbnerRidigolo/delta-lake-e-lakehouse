...
def _transaction_aggregates(transactions: DataFrame, reference_date) -> DataFrame:
    ...
    return transactions.groupBy("customer_id").agg(
        ...
        F.sum(F.when(F.col("event_date") >= F.date_sub(reference_date, 90), 1).otherwise(0))
         .alias("txn_count_90d"),
        F.round(F.sum(F.when((F.col("event_date") >= F.date_sub(reference_date, 90)) & approved,
                             F.col("amount")).otherwise(0.0)), 2).alias("tpv_90d"),
        F.round(F.sum(F.when((F.col("event_date") >= F.date_sub(reference_date, 30)) & approved,
                             F.col("amount")).otherwise(0.0)), 2).alias("tpv_30d"),
    )
...
def _credit_aggregates(contracts: DataFrame) -> DataFrame:
    ...
    return contracts.groupBy("customer_id").agg(
        ...
        F.sum(F.when(F.col("status").isin("active", "default"), 1).otherwise(0))
         .alias("active_contracts_count"),
...
def build(spark: SparkSession) -> DataFrame:
    ...
    base = (
        customers.select(
            "customer_id", "customer_key", "full_name", "cpf_masked", "email_masked",
            "city", "state", "region", "segment", "risk_band", "status", "age", "age_band",
            "monthly_income", "income_band", "credit_limit", "limit_to_income_ratio",
            "signup_date", "signup_cohort", "tenure_days", "tenure_band", "acquisition_channel",
        )
    ...
    enriched = (
        ...
        .withColumn("total_exposure",
                    F.round(F.coalesce(F.col("credit_outstanding_total"), F.lit(0.0)), 2))
        .withColumn("limit_utilization",
    ...
    scored = (
        enriched
        .withColumn("customer_score", F.round(_customer_score(), 0).cast("int"))
        .withColumn("rating",
        ...
        .withColumn(
            "value_risk_segment",
            F.when((F.col("tpv_90d") > 5000) & (F.col("customer_score") >= 700), "alto_valor_baixo_risco")
             .when((F.col("tpv_90d") > 5000) & (F.col("customer_score") < 700), "alto_valor_alto_risco")
    ...
    zero_fill = ["txn_count_total", "txn_count_90d", "tpv_total", "tpv_90d", "tpv_30d",
                 "chargeback_count", "confirmed_fraud_count", "distinct_merchants",
                 "distinct_devices", "credit_contracts_count", "credit_outstanding_total",
                 "credit_provision_total", "max_days_past_due", "mdr_revenue_generated"]
...
def run() -> int:
    ...
    with log_stage(log, f"gold.{TABLE}"):
        ...
        write_delta(gold, path, mode="overwrite", partition_by=["segment"],
                    overwrite_schema=True, comment="silver -> gold.customer_360")
        ...
        count = result.count()
        log.info("gold.%s: %s clientes | churn=%s | NPL=%s", TABLE, count,
