...
def _transaction_kpis(transactions: DataFrame) -> DataFrame:
    ...
    return transactions.groupBy("event_date").agg(
        ...
        F.round(
            F.sum(F.when((F.col("is_fraud") == 1) & approved, F.col("amount")).otherwise(0.0)), 2
        ).alias("loss_fraud"),
...
def _credit_daily(spark: SparkSession, contracts: DataFrame, calendar: DataFrame) -> DataFrame:
    ...
    dated = (
        contracts.withColumn(
            "first_default_date",
            F.when(F.col("days_past_due") > 0, F.date_sub(reference_date, F.col("days_past_due"))),
        )
        .withColumn(
            "write_off_date",
            F.when(
                F.col("is_written_off"), F.date_add(F.col("first_default_date"), WRITE_OFF_LAG_DAYS)
            ),
        )
        .withColumn(
            "recovery_date",
            F.when(
                F.col("recovery_amount") > 0, F.date_add(F.col("write_off_date"), RECOVERY_LAG_DAYS)
            ),
        )
    )
    ...
    accrual = (
        calendar.join(
            dated.where(F.col("status").isin("active", "default")).select(
                "contract_id", "origination_date", "outstanding_balance", "interest_rate_month"
            ),
        ...
        .agg(
            F.round(
                F.sum(F.col("outstanding_balance") * F.col("interest_rate_month") / 30.0), 2
    ...
    write_offs = (
        ...
        .agg(
            F.round(F.sum("outstanding_balance"), 2).alias("loss_write_off"),
    ...
    return accrual.join(write_offs, on="event_date", how="full_outer").join(
        recoveries, on="event_date", how="full_outer"
    )
...
def build(spark: SparkSession) -> DataFrame:
    ...
    bounds = transactions.agg(
        F.min("event_date").alias("start"), F.max("event_date").alias("end")
    ).collect()[0]
    calendar = spark.sql(
    ...
    daily = calendar.join(txn_kpis, on="event_date", how="left").join(
        credit_kpis, on="event_date", how="left"
    )
    ...
    numeric_defaults = {
        ...
        "pix_share": 0.0,
        "cnp_share": 0.0,
        "revenue_interest": 0.0,
        "credit_portfolio_balance": 0.0,
        "active_contracts": 0,
        "loss_write_off": 0.0,
    ...
    return (
        ...
        .withColumn(
            "loss_total",
            F.round(F.col("loss_chargeback") + F.col("loss_fraud") + F.col("loss_write_off"), 2),
        )
        .withColumn(
            "net_result",
            F.round(F.col("revenue_total") - F.col("loss_total") + F.col("recovery_amount"), 2),
        )
        .withColumn(
            "net_margin",
        ...
        .withColumn(
            "take_rate", F.round(F.col("revenue_mdr") / F.greatest(F.col("tpv"), F.lit(0.01)), 5)
        )
...
def run() -> int:
    ...
    with log_stage(log, f"gold.{TABLE}"):
        ...
        write_delta(
            ...
            comment="silver -> gold.financial_kpis_daily",
        )
        ...
        log.info(
            "gold.%s: TPV=R$ %s | receita=R$ %s | perdas=R$ %s | recuperacao=R$ %s | "
            "resultado=R$ %s",
            TABLE,
            totals["tpv"],
            totals["receita"],
