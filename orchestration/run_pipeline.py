...
PIPELINE: List[Stage] = [
    Stage("generate_data", "Gera os dados sinteticos da fintech na camada raw",
          _generate_data, []),

    Stage("bronze_batch", "Ingestao batch dos CSVs raw -> Bronze (Delta)",
          _bronze_batch, ["generate_data"]),
    Stage("bronze_stream", "Ingestao streaming dos eventos JSONL -> Bronze",
          _bronze_stream, ["bronze_batch"]),

    Stage("silver_customers", "Limpeza, LGPD e enriquecimento de clientes",
          _silver_customers, ["bronze_batch"]),
    Stage("silver_merchants", "Limpeza e taxonomia de MCC dos estabelecimentos",
          _silver_merchants, ["bronze_batch"]),
    Stage("silver_transactions", "Deduplicacao, validacao e enriquecimento de transacoes",
          _silver_transactions, ["bronze_stream", "silver_customers", "silver_merchants"]),
    Stage("silver_credit_contracts", "Faixas de atraso, provisao e safra dos contratos",
          _silver_credit_contracts, ["bronze_batch", "silver_customers"]),

    Stage("gold_customer_360", "Visao 360 do cliente com score e churn",
          _gold_customer_360, ["silver_transactions", "silver_credit_contracts"]),
    Stage("gold_fraud_signals", "Sinais e score de fraude por transacao",
          _gold_fraud_signals, ["silver_transactions"]),
    Stage("gold_credit_risk", "Carteira de credito por safra (PD/LGD/EL)",
          _gold_credit_risk, ["silver_credit_contracts"]),
    Stage("gold_financial_kpis", "KPIs financeiros diarios (receita, perdas, recuperacao)",
          _gold_financial_kpis, ["silver_transactions", "silver_credit_contracts"]),
    Stage("gold_merchant_performance", "Performance e risco por estabelecimento",
          _gold_merchant_performance, ["silver_transactions", "silver_merchants"]),

    Stage("feature_store", "Materializa as feature views (point-in-time)",
          _feature_store, ["gold_fraud_signals", "gold_customer_360"]),
    Stage("train_fraud_model", "Treina e avalia o modelo de fraude",
          _train_fraud_model, ["feature_store"]),

    # Depende tambem do treino: o relatorio incorpora as metricas do modelo.
    Stage("ai_insights", "Gera o relatorio analitico automatico da Gold",
          _ai_insights, ["gold_financial_kpis", "gold_credit_risk",
                         "gold_customer_360", "gold_merchant_performance",
                         "train_fraud_model"]),
    Stage("rag_index", "Indexa a camada Gold no vetor store para o RAG",
          _rag_index, ["ai_insights"]),

    Stage("catalog_metadata", "Aplica os metadados de governanca nas tabelas Delta",
          _catalog_metadata, ["gold_customer_360", "gold_financial_kpis"], critical=False),
    Stage("delta_maintenance", "OPTIMIZE / Z-ORDER / VACUUM das tabelas Delta",
          _delta_maintenance, ["catalog_metadata"], critical=False),
]
...
def topological_order(stages: List[Stage]) -> List[Stage]:
    ...
    while pending:
        ready = sorted(name for name, deps in pending.items() if not (deps & available & set(pending)))
        if not ready:
...
def select_stages(start_from: Optional[str], only: Optional[List[str]],
                  skip: Optional[List[str]]) -> List[Stage]:
    ...
    trimmed = [
        Stage(s.name, s.description, s.runner,
              [d for d in s.depends_on if d in selected], s.retries, s.critical)
        for s in stages
    ]
...
def run_stage(stage: Stage) -> Dict:
    ...
    while True:
        ...
        try:
            ...
            return {"stage": stage.name, "status": "sucesso", "duracao_s": round(elapsed, 1),
                    "tentativas": attempt, "resultado": _summarize(result)}
        except Exception as exc:
            if attempt > stage.retries:
                elapsed = time.perf_counter() - started
                log.error("!!! %s FALHOU apos %s tentativa(s): %s", stage.name, attempt, exc)
                return {"stage": stage.name, "status": "falha", "duracao_s": round(elapsed, 1),
                        "tentativas": attempt, "erro": str(exc)[:400]}
            backoff = 2 ** attempt
...
def run_pipeline(start_from: Optional[str] = None, only: Optional[List[str]] = None,
                 skip: Optional[List[str]] = None, stop_on_failure: bool = True) -> List[Dict]:
...
def _print_report(report: List[Dict], elapsed: float, run_id: str) -> None:
    ...
    for row in report:
        log.info("%s %-26s %6ss  %s", icons.get(row["status"], "[?]"), row["stage"],
                 row.get("duracao_s", "-"), row.get("resultado", row.get("erro", "")))
...
def main() -> None:
    ...
    parser.add_argument("--from", dest="start_from", default=None,
                        help="executa a partir desta etapa")
    parser.add_argument("--only", action="append", default=None,
                        help="executa apenas esta etapa (pode repetir)")
    parser.add_argument("--skip", action="append", default=None,
                        help="pula esta etapa (pode repetir)")
    ...
    report = run_pipeline(args.start_from, args.only, args.skip,
